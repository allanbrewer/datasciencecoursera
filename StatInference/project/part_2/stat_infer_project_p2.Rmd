...
After doing the hypothesis tests we can determine that for 0.5 and 1 milligram per day has a greater effect on tooth length and for 2 milligrams it appers to that the mean stays equal. The other tests prove that the difference in lenght achived by a greater dose achives greater tooth growth is statisticaly significant.
...
For this section we will perform various Hypothesis test to understand if the difference in length between Supplement Type and Dose is statistically significant. We will perform 5 different Hypothesis Test using the T Test. The Test will be done with the following characterisitcs. Assume unequal variance, unpaired data and we will do a two sided test for two different variables to determine if the means difference is statistically significant. The R code for the test can be found in the appendix under "Hypothesis Test R Code"
...
* P-Value:  
```{r hypo1_conf, echo=FALSE}
hypo1$p.value
```
* Result: With the P-value reported above we reject the null hypothesis and say that the difference in mean different to 0 is statistically significant.
...
From the T Test and Hypothesis tests done with the above assumptions we can determine that the greater the dose the greater the tooth length and that for 0.5 and 1 milligram per day the OJ has a greater effect in tooth length than VC, for 2 milligrams per day the effect of OJ and VC seem to be equal on tooth growth.
