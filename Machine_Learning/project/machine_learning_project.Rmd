...
output:
  pdf_document:
    ...
    fig_width: 6
    fig_height: 4
subtitle: Predict the quality of Weight Lifting Exercises using machine learning in R.
fontsize: 10pt
geometry: margin = 1in
---
...
In this project we will train a machine learning model to determine if weight lifts are done correctly. We must choose the varaibles and algorithim to use. The results must be validated against a testing data submited to the Coursera Project Submission system.
...
Firts we load the data sets and identify the different values that should be considered NA and discaded in our analysis.
...
After cleaning the data we end up with a data set that can be used to train the model, a part of the set will be hold out to be tested later and determine the out of sample error. In the next section we will train the model and apply Bootstrap Cross Validation using the train_control function. We will set the model to train 10 Random Forest using cross validation, each Forest will contain 100 trees and each will be trained twice sampling 10 variables for each tree and sampling 26 varaibles.
