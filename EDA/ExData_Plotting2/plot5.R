...
png(file = "plot5.png", width = 600, height = 600, units = "px", bg = "transparent")
## Create the plot in current graphic device
plot5 <- ggplot(nei.mv.bc.sum, aes(year, total_emissions))
plot5 <- plot5 + labs(title = "Emissions from Motor Vehicle Sources in Baltimore City per Year", x = "Years", y = "Total Emissions (tons)")
